...
static void video_fg_display(VideoState *is, double phase)
{
    ...
    fsr_toast_draw(renderer);
    fsr_hud_draw(renderer);
    fg_debug_marker(drew);             /* red = generated frame, green = repeat */
    SDL_RenderPresent(renderer);
    fps_tick();
}
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_g:
                ...
                break;
            case SDLK_i:
                /* Debug: corner marker blinks red (generated) / green (real) so
                 * you can see whether FG is actually producing frames. */
                fg_debug = !fg_debug;
            ...
            case SDLK_DELETE: {
                 ...
                 *
                 * The .nfo, .jpg and numbered screenshots named after the
                 * video go with it - see fsr_delete_file - since they are
                 * orphans once it is gone.
                 *
