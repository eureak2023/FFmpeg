...
static int hwgl_ensure_size(int w, int h)
{
    ...
    /* ...except while VSR is wanted, because VSR *is* driver-side video
     * processing and this switch turns it off with everything else. Nothing
     * says so: the extension call still returns S_OK, the model just never
     * runs and the processor scales with plain bilinear. That is measurable -
     * with auto processing off, the 2x output matched an OpenCV bilinear
     * upscale of the same frame to 0.006 levels, which is the same picture,
     * not a similar one. With it on the model runs: high-frequency energy at
     * the slot went 3.58 -> 14.97 (bilinear 3.58, lanczos4 6.45).
     *
     * The colour boost the paragraph above is about did not come back with
     * it. Measured against each run's own source frame so content cancels:
     * per-channel gain within 0.5% of the auto-processing-off path (B 1.011
     * vs 1.014, G 1.126 vs 1.131, R 1.031 vs 1.036), mean saturation 0.917
     * vs 0.920, and clipped pixels actually lower. */
    ID3D11VideoContext_VideoProcessorSetStreamAutoProcessingMode(hwgl.vcontext,
        hwgl.vp, 0, want_scale ? TRUE : FALSE);
...
static int hwgl_convert(AVFrame *frame, int slot)
{
    ...
    if (hwgl.vcontext1) {
        ...
        if (frame->color_trc == AVCOL_TRC_SMPTE2084) {
             ...
             * the transfer and the primaries match on input and output, leaving
             * the VP nothing to convert but the BT.2020 YCbCr de-matrix and the
             * studio->full range expansion.
             *
             * This previously tagged both sides as G22 to fake the no-op, but
             * on current NVIDIA drivers that lie makes the VP convert primaries
             * on its own, which the shader then repeats. Reading the VP's output
             * texture back and fitting it against swscale's conversion of the
             * same frame returns the BT.2020->BT.709 matrix (1.586/-0.506/-0.074
             * per row against its 1.661/-0.588/-0.073); with the G2084 tag the
             * same fit returns identity. The doubled conversion over-saturated
             * everything and pushed warm tones toward red - a headless A/B put
             * forehead PQ RGB at 0.273/0.162/0.079 against swscale's correct
             * 0.233/0.169/0.092. Set FSR_DEBUG_DUMP to repeat that measurement. */
